...
descriptors_train=np.swapaxes(descriptors[:8000],0,1)
descriptors_test=np.swapaxes(descriptors[8000:10000],0,1)


energies_train=energies[:8000]
energies_train = scale(energies_train[:,np.newaxis])
energies_test=energies[8000:10000]
...
energies_pred=model_TOT.predict(list_descriptor_test)
plt.plot(energies_test,energies_pred,'.')
plt.plot(energies_test,energies_test,'-')
plt.xlabel('true value')
plt.ylabel('guess')
plt.savefig('energie_guess')

plt.figure(figsize=[10, 5])
plt.plot(history_TOT.history['loss'])
plt.plot(history_TOT.history['val_loss'])
